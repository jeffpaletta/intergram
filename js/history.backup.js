...
AdImpression.prototype = {
    ...
    focus:function(){
        ...
        var s = '<p class="adInfo"><span class="date"></span><br/><span class="publisher"></span><br/><span class="format"></span><span class="color"></span><span class="theme"></span><div class="flag"><p>Remove ad / edit tags</p></div>';

        d3.select('#container div.info').html(s);
        d3.select('#container div.info span.date').text(d);
...
FilterManager = function(){
    ...
    for(f in this.list){
        d3.select(this.list[f].svg.node().parentNode)
            ...
            .on('mouseover',function(){d3.select(this).transition().duration(200).ease("cubic-out").style('background-color','#2B2B2B')})
            .on('mouseout',function(){d3.select(this).transition().duration(200).ease("cubic-out").style('background-color','#252525')})
    }
...
SortMode.prototype = {
    ...
    init: function(){
        ...
        var _this = this;
        this.ready = true;
        var modes = ['By ad placement', 'Most recent', 'Most uncommon'];
        ...
        d3.select('g.sortModes').selectAll('g.modes')
            ...
            .on('mouseout',function(d,i){
                ...
                d3.select(this).select('rect.bkg')
                    .transition()
                    .duration(150)
                    .attr('fill','rgba(255,190,3,' + (filtering ? 0.3 : 0.0) + ')')
            })
            .each(function(d,i){
                d3.select(this)
                    .append('rect')
...
AdLoader.prototype = {
    ...
    init: function(){
        ...
        d3.select('#container')
            ...
            .append('p')
                .text("Hold on, we're looking for your ads.")
                .style('width',(w*4)+'px')
                .style('margin',0)
                .style('position','absolute')
                .style('bottom',0)
        ...
        svg.append('path')
            ...
            .attr('fill','rgba(247,190,15,0.4)')

        svg.append('path')
            .classed('arc',true)
            .attr('d', arcConstructor)
            .attr('fill','rgba(247,190,15,0.5)')

        svg.append('path')
            .classed('arc',true)
            .attr('d', arcConstructor2)
            .attr('fill','rgba(247,190,15,0.25)')
